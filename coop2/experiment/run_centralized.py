...
with the following decision flow:
    ...
    Leader:
        1. Send a planning request to all followers
        2. Wait for status/proposal responses from followers
        3. Generate plan using LLM from follower responses
    
    Follower:
        1. Wait for leader planning request
        2. Respond with local status/proposal
        3. Generate a feasible local plan
"""
...
def run_centralized_experiment(
    ...
    """
    print("="*80)
    print("CENTRALIZED TOPOLOGY WITH LLM AGENTS")
    print("="*80)
    ...
    # Create centralized LLM agents
    print(f"\nCreating centralized topology with {n_agents} agents...")
    print(f"  Leader: agent_0")
    print(f"  Followers: {agent_names[1:]}")
    ...
    agents = create_centralized_llm_agents(
        n_agents=n_agents,
        llm_client=llm_client,
        temperature=0.7,
        verbose=verbose,
    ...
    plan_env = PlanningEnvWrapper(
        ...
        interrupt_on_message=True,  # Important: enable message interrupts
    ...
    try:
        while env.current_step < max_steps and not done:
            ...
            # Wait until all agents are ready (trigger reasoning/planning via threads)
            ...
            # Environment handles agent coordination
    ...
    print(f"  - plan_logs.json: Plan execution logs")
    print(f"  - agent_states.json: Agent state timelines")
    print(f"  - agent_timeline.png: Per-agent FSM state over wall clock")
    print(f"  - comprehensive_timeline.png: Full timeline with messages")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run centralized LLM experiment")
